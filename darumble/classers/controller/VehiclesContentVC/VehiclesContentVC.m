...
@interface VehiclesContentVC ()
{
    ...
    IBOutlet UILabel *label_no_vehicle;
}
...
@implementation VehiclesContentVC
...
- (void)viewDidLoad {
    ...
    if ([[USER_DEFAULT objectForKey:@"DaRumble_Login"] intValue] != 0) {
        ...
        if (self.flag_global_local == 1)
        {
            ...
            [ws load_local_vehicles:APP_TOKEN zip:str userID:[[USER_DEFAULT objectForKey:@"userID"] intValue]];
        }else
            [ws load_global_vehicles:APP_TOKEN userID:self.filtered_userId];
    }
...
//---collectionView---
    UINib *cellNib = [UINib nibWithNibName:@"VehicleCell" bundle:nil];
    [collectView registerNib:cellNib forCellWithReuseIdentifier:@"VehicleCell"];
...
{
    persize *= 2;
    if (persize > vehiclesArray.count)
        persize = vehiclesArray.count;
...
-(UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    VehicleCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:cellIdentifier forIndexPath:indexPath];
    PhotoObj *obj = [vehiclesArray objectAtIndex:indexPath.row];
    ...
    [Common downloadImageWithURL:[NSURL URLWithString: obj.URL] completionBlock:^(BOOL succeeded, UIImage *image) {
        if (succeeded) {
            VehicleCell *updateCell = (VehicleCell*)[collectionView cellForItemAtIndexPath:indexPath];
...
-(void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    vc.photoObj = [vehiclesArray objectAtIndex:indexPath.row];
...
- (void)addNotifi
{
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(Get_vehicle_success:) name:k_globalvehicles_success object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(Get_vehicle_fail:) name:k_globalvehicles_fail object:nil];
}
- (void)removeNotifi
{
    [[NSNotificationCenter defaultCenter] removeObserver:self name:k_globalvehicles_success object:nil];
    [[NSNotificationCenter defaultCenter] removeObserver:self name:k_globalvehicles_fail object:nil];
}
-(void) Get_vehicle_success:(NSNotification*)notif
{
    NSDictionary *dic = notif.object;
    if ([[dic objectForKey:@"code"] intValue] == 200) {
        NSArray *arr = [[dic objectForKey:@"data"] objectForKey:@"vehicles"];
        [vehiclesArray removeAllObjects];
        [collectView reloadData];
        ...
        for (NSDictionary *s in arr)
        {
            ...
            {
                if (self.filtered_userId != obj.uid)
                    continue;
            }
            
            [vehiclesArray addObject:obj];
        ...
        if ([vehiclesArray count] == 0)
        {
            ...
            label_no_vehicle.hidden = NO;
        }else{
            label_no_vehicle.hidden = YES;
        }
...
-(void) Get_vehicle_fail:(NSNotification*)notif
{
    [MBProgressHUD hideAllHUDsForView:self.view animated:YES];
    [self removeNotifi];
}
