...
- (void)viewDidLoad {
    ...
    if ([[USER_DEFAULT objectForKey:@"country"] isEqualToString:@""] || [[USER_DEFAULT objectForKey:@"country"] isEqualToString:@"(null)"])
    {
        [Common showAlert:@"Your zipcode or country is empty. So local data is not rich now."];
    }else if ([[USER_DEFAULT objectForKey:@"zip"] isEqualToString:@""] || [[USER_DEFAULT objectForKey:@"zip"] isEqualToString:@"(null)"])
    {
        [Common showAlert:@"Your zipcode or country is empty. So local data is not rich now."];
    }
    
    [ws load_local_feed:APP_TOKEN geolocation:([USER_DEFAULT objectForKey:@"country"]!=nil)?[USER_DEFAULT objectForKey:@"country"]:@"" zip:([USER_DEFAULT objectForKey:@"zip"]!=nil)?([USER_DEFAULT objectForKey:@"zip"]):(@"") userID:[[USER_DEFAULT objectForKey:@"userID"] intValue]];
...
- (void)onBannerClick:(UITapGestureRecognizer *)gesture
{
    ...
    obj = self.bannerImages[m_bannerIndex];
    if (obj.id == 0){
...
//            [self.navigationController pushViewController:vc animated:YES];
        ...
        case 16:
        {
            ...
            NSLog([NSString stringWithFormat:@"test*:%d",obj.uid]);
...
- (void)addNotifi
{
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(GetLocalFeed_Success:) name:k_localfeed_success object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(GetLocalFeed_Fail:) name:k_localfeed_fail object:nil];
}
- (void)removeNotifi
{
    [[NSNotificationCenter defaultCenter] removeObserver:self name:k_localfeed_success object:nil];
    [[NSNotificationCenter defaultCenter] removeObserver:self name:k_localfeed_fail object:nil];
}

-(void) GetLocalFeed_Success:(NSNotification*)notif{
    NSDictionary *dic = notif.object;
#if DEBUG
    NSLog(@"LocalFeed SUCCESS %@",dic);
#endif
...
-(void) GetLocalFeed_Fail:(NSNotification*)notif{
