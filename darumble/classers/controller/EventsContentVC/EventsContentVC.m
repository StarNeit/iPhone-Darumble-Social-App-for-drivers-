...
@interface EventsContentVC ()
{
    ...
    IBOutlet UILabel *label_no_events;
}
...
@implementation EventsContentVC
...
- (void)viewDidLoad {
    ...
    if ([[USER_DEFAULT objectForKey:@"DaRumble_Login"] intValue] != 0)
    {
        ...
        if (self.flag_global_local == 1)
        {
            ...
            [ws load_local_events:APP_TOKEN zip:str userID:[[USER_DEFAULT objectForKey:@"userID"] intValue]];
        }else
            [ws load_global_events:APP_TOKEN userID:self.filtered_userId];
    }
    ...
    //collectionView
    UINib *cellNib = [UINib nibWithNibName:@"EventsCell" bundle:nil];
    [collectView registerNib:cellNib forCellWithReuseIdentifier:@"EventsCell"];
...
- (void)refresh
{
    ...
    if (eventsArray.count < persize)
        persize = eventsArray.count;
...
-(UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    EventsCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:cellIdentifier forIndexPath:indexPath];
    PhotoObj *obj = [eventsArray objectAtIndex:indexPath.row];
    ...
    [Common downloadImageWithURL:[NSURL URLWithString: obj.URL] completionBlock:^(BOOL succeeded, UIImage *image) {
        if (succeeded) {
            EventsCell *updateCell = (EventsCell*)[collectionView cellForItemAtIndexPath:indexPath];
...
-(void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    vc.photoObj = [eventsArray objectAtIndex:indexPath.row];
...
- (void)addNotifi
{
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(GetEvents_Success:) name:k_globalevents_success object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(GetEvents_Fail:) name:k_globalevents_fail object:nil];
}
- (void)removeNotifi
{
    [[NSNotificationCenter defaultCenter] removeObserver:self name:k_globalevents_success object:nil];
    [[NSNotificationCenter defaultCenter] removeObserver:self name:k_globalevents_fail object:nil];
}
-(void) GetEvents_Success:(NSNotification*)notif
{
    NSDictionary *dic = notif.object;
    if ([[dic objectForKey:@"code"] intValue] == 200) {
        NSArray *arr = [[dic objectForKey:@"data"] objectForKey:@"events"];
        ...
        if ([eventsArray count] == 0)
        {
            ...
            label_no_events.hidden = NO;
        }else{
            label_no_events.hidden = YES;
        }
