...
@interface VideoDetailsVC ()
{
    ...
    IBOutlet UILabel *label_video_title;
...
//---WS API Call---//
    ...
    {
    //---WS API Call(Video Details)---//
        [self removeNotifi];
        [self addNotifi];
        [MBProgressHUD showHUDAddedTo:self.view WithTitle:@"Waiting..." animated:YES];
        DataCenter *ws = [[DataCenter alloc] init];
        [ws get_video_details:APP_TOKEN videoID:self.photoObj.id userID:[[USER_DEFAULT objectForKey:@"userID"] intValue]];
...
- (void)addNotifi
{
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(GetVideoDetail_Success:) name:k_video_details_success object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(GetVideoDetail_Fail:) name:k_video_details_fail object:nil];
}
- (void)removeNotifi
{
    [[NSNotificationCenter defaultCenter] removeObserver:self name:k_video_details_success object:nil];
    [[NSNotificationCenter defaultCenter] removeObserver:self name:k_video_details_fail object:nil];
}

-(void) GetVideoDetail_Success:(NSNotification*)notif{
    NSDictionary *dic = notif.object;
#if DEBUG
    NSLog(@"GetVideoDetail_Success %@",dic);
#endif
    if ([[dic objectForKey:@"code"] intValue] == 200)
    {
        NSDictionary *data = [dic objectForKey:@"data"];
        NSDictionary *video_details = [data objectForKey:@"video_details"];
        NSString *description = [video_details objectForKey:@"description"] != [NSNull null] ?[video_details objectForKey:@"description"] : @"";
        NSString *title = [video_details objectForKey:@"title"] != [NSNull null] ?[video_details objectForKey:@"title"] : @"";
        NSString *fname = [video_details objectForKey:@"fname"] != [NSNull null] ?[video_details objectForKey:@"fname"] : @"";
        NSString *lname = [video_details objectForKey:@"lname"] != [NSNull null] ?[video_details objectForKey:@"lname"] : @"";
        
        int uid = [video_details objectForKey:@"uid"] != [NSNull null] ? [[video_details objectForKey:@"uid"] intValue] : 0;
        int is_flagged = [video_details objectForKey:@"is_flagged"] != [NSNull null] ? [[video_details objectForKey:@"is_flagged"] intValue] : 0;
        int is_like = [video_details objectForKey:@"is_like"] != [NSNull null] ? [[video_details objectForKey:@"is_like"] intValue] : 0;
        
        btn_flag.tag = is_flagged;
        btn_flag.tag = is_like;
        [self setLikeFlagButtons];
        
         [tv_description setTextColor:[UIColor whiteColor]];
         tv_description.text = description;
        label_video_title.text = title;
...
-(void) GetVideoDetail_Fail:(NSNotification*)notif{
...
-(void) FlagContent_Success:(NSNotification*)notif
{
    ...
    if ([[dic objectForKey:@"code"] intValue] == 200)
    {
        ...
        /*if (isFlag == 1)
        {
            [Common showAlert:@"You have unflagged this video"];
        }else if (isFlag == 2)
        {
            [Common showAlert:@"You have flagged this video"];
        }*/
        [Common showAlert:@"Staff will be notified. Thank you"];
    }else
    {
